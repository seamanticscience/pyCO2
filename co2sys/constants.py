...
def calculate_KF(salt, temp, pres=None,ver="Dickson"):
    """Calculate the dissociation constant of Hydrogen Fluoride"""
    if "dick" in ver.lower():
        ...
        KF = np.exp(lnKF) # the free pH scale in mol/kg-SW
    if pres is not None:
        Pbar = pres / 10
...
def millero_consensus(salt, temp, pres=None):
    ...
    lnK1 = lnK1 + ( -0.138681 - 9.33291 / tempK) * salt**0.5 + 0.0726483 * salt - 0.00574938 * salt**1.5
    lnK2 =  -0.84226 - 3741.1288 / tempK - 1.437139 * logTempK 
    lnK2 = lnK2 + (-0.128417 - 24.41239 / tempK) * salt**0.5 +  0.1195308 * salt - 0.00912840 * salt**1.5
    K1 = np.exp(lnK1) # SWS pH scale in mol/kg-SW
    K2 = np.exp(lnK2) # SWS pH scale in mol/kg-SW
    if pres is not None:
        K1,K2 = calculate_press_effects_on_K1_K2(K1, K2, temp, pres)
    cdict['K1'] = K1
    cdict['K2'] = K2
...
def waters_2014(salt, temp, pres=None):
    ...
    pK1 = pK1 + (-531.3642 * salt**0.5 - 5.713 * salt) / tempK - 2.0669166 * salt**0.5 * logTempK
    K1   = 10**-pK1
    pK2  = 5143.692 / tempK + 14.613358 * logTempK - 90.18333 + 21.225890 * salt**0.5 + 0.12450870 * salt - 3.7243e-4 * salt**2
    pK2  = pK2 + (-779.3444 * salt**0.5 - 19.91739 * salt) / tempK - 3.3534679 * salt**0.5 * logTempK
    K2   = 10**-pK2
    if pres is not None:
        K1,K2 = calculate_press_effects_on_K1_K2(K1, K2, temp, pres)
    cdict['K1'] = K1
